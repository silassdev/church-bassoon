...
async function loadFromFs(templateName, locale) {
...
async function loadTemplate(templateName:string, locale:string) {
  ...
  const override = await EmailTemplateOverride.findOne({ templateName, locale }).lean();
...
function renderTemplates({ subject, html, text }, vars = {}) {
